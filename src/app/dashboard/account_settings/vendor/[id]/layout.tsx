...
const VendorDetailsLayout = ({children, params}: {children: React.ReactNode; params: Promise<{ id: string }>;}) => {
  ...
  const tabs =[
    {name: "All Content", path: `/dashboard/account_settings/vendor/${id}/contents`},
    {name: "Transaction History", path: `/dashboard/account_settings/vendor/${id}/transactions`},
    {name: "Activity Log", path: `/dashboard/account_settings/vendor/${id}/activity`}
]
  ...
  return (
    <div className='p-4'>
        <div className="flex items-center text-[#6D6D6D] text-sm"><MdArrowBackIosNew /> <span onClick={() => router.push('/dashboard/account_settings/vendor')} className="text-blue-600 cursor-pointer">Vendor Management</span>/{singleVendor?.personnel_name}</div>
            ...
            <div className="flex gap-2">
                ...
                <div className="flex flex-col gap-8">
                    <ul className="space-y-2">
                        <li className="font-bold text-[#151515] text-2xl">{singleVendor?.personnel_name}</li>
                        <li className="font-bold text-red-500 text-sm">{singleVendor?.type}</li>
                    </ul>
                </div>
            </div>
            <div className="grid grid-cols-2 gap-4">
                <ul className="flex gap-8 justify-between bg-[#F9FCFF] border w-[217px] h-[84px] p-2 rounded-lg">
                    <li className="flex flex-col text-[#2F4858] text-2xl">
                        ...
                        {singleVendor?.total_content_download || 0}
                    </li>
                    <li>
                ...
                <ul className="flex gap-8 justify-between bg-[#FD9F9F40] text-[#F42727] border w-[217px] h-[84px] p-2 rounded-lg">
                    <li className="flex flex-col text-[#2F4858] text-2xl">
                        ...
                        <span className="flex items-center">
                            ...
                            {singleVendor.orders?.total_amount_spent || 0}
                        </span>
                    </li>
                    <li>
